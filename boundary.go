...
//
// Endpoint is a Boundary wrapper that allows arbitrary types of intervals.
//
// NOTE: a completely equivalent way of thinking about this is to think of
// Boundaries as being infinitesimal and not corresponding to any valid value.
// In this case, there is no concept of inclusive or exclusive intervals.
// Endpoint can represent infinitesimal boundaries that are immediately before
// or after a value, allowing arbitrary types of intervals.
//
// The boundary (together with a data structure using boundaries) can be
// conceptualized in two equivalent ways:
//
//   - the boundary itself does not correspond to a valid value, and is an
//     infinitesimal boundary between valid values. For example, if the objects
//     we're dealing with are positive integers, the boundaries could be values
//     like 0.5, 1.5, 2.5, 3.5, etc.
//
//   - the boundary can correspond to an actual value, and the data structure deals
//     with half-open intervals, like [1, 2).
//
// In both cases, intervals that touch at a boundary can be fused.
type Boundary any
